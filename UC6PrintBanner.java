/**
 * UC6PrintBanner – Banner Display Application (Use Case 6)
 * This class refactors UC5 by utilizing static helper methods to 
 * generate character patterns, adhering to the DRY principle.
 * @author Divyansh Sanadhya
 * @version 6.0
 */
public class UC6PrintBanner {
    public static void main(String[] args) {
        // 1. Populate the banner array by invoking the helper methods
        // Notice how getOPattern() is called twice—reusability in action!
        String[] lines = {
            String.join(" ", getOPattern(0), getOPattern(0), getPPattern(0), getSPattern(0)),
            String.join(" ", getOPattern(1), getOPattern(1), getPPattern(1), getSPattern(1)),
            String.join(" ", getOPattern(2), getOPattern(2), getPPattern(2), getSPattern(2)),
            String.join(" ", getOPattern(3), getOPattern(3), getPPattern(3), getSPattern(3)),
            String.join(" ", getOPattern(4), getOPattern(4), getPPattern(4), getSPattern(4)),
            String.join(" ", getOPattern(5), getOPattern(5), getPPattern(5), getSPattern(5)),
            String.join(" ", getOPattern(6), getOPattern(6), getPPattern(6), getSPattern(6))
        };

        // 2. Loop-based rendering for the final output
        for (String line : lines) {
            System.out.println(line);
        }
    }

    /** Helper method for letter 'O' pattern */
    public static String getOPattern(int row) {
        String[] pattern = {"  *** ", " ** **", " ** **", " ** **", " ** **", " ** **", "  *** "};
        return pattern[row];
    }

    /** Helper method for letter 'P' pattern */
    public static String getPPattern(int row) {
        String[] pattern = {" ****** ", " ** ** ", " ** ** ", " ****** ", " ** ", " ** ", " ** "};
        return pattern[row];
    }

    /** Helper method for letter 'S' pattern */
    public static String getSPattern(int row) {
        String[] pattern = {"  ***** ", " ** ", " ** ", "  *** ", "    ** ", "    ** ", " ***** "};
        return pattern[row];
    }
}